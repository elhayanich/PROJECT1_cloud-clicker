...
// Fonction qui ajoute un checkmark à coté du palier dés qu'il est atteint + animations
function checkStep() {
  if (!reachedStep1) {
    if (score >= steps[0] && score < steps[1]) {
      reachedStep1 = true;
      body.backgroundImage.style.backgroundImage = 'url(./assets/Background-2.png)';
      scoreMultiplier = 2;
  ...
  if (!reachedStep2) {
    if (score >= steps[1] && score < steps[2]) {
      reachedStep1 = true;
      reachedStep2 = true;
      body.backgroundImage.style.backgroundImage = 'url(./assets/background-3.png)';
      scoreMultiplier = 4;
  ...
  if (!reachedStep3) {
    if (score >= steps[2] && score < steps[3]) {
      ...
      reachedStep2 = true;
      reachedStep3 = true;
      body.backgroundImage.style.backgroundImage = 'url(./assets/background-4.png)';
      scoreMultiplier = 8;
...
// ? fonction qui remplace la valeur par défaut du score si une valeur existe dans localStorage
function checkScore() {
...
// ? fonction qui vérifie si des bonus sont actifs et les réactivent
function checkBonuses() {
  if (!isNaN(localStorage.getItem('bonus-1')) && localStorage.getItem('bonus-1') != null) {
    bonus1.storedAmount = parseInt(localStorage.getItem('bonus-1'), 10);
    bonus1.amount = bonus1.storedAmount;
    activeBonus();
  }
  if (!isNaN(localStorage.getItem('bonus-2')) && localStorage.getItem('bonus-2') != null) {
    bonus2.storedAmount = parseInt(localStorage.getItem('bonus-2'), 10);
    bonus2.amount = bonus2.storedAmount;
    activeBonus();
  }
  if (!isNaN(localStorage.getItem('bonus-3')) && localStorage.getItem('bonus-3') != null) {
    bonus3.storedAmount = parseInt(localStorage.getItem('bonus-3'), 10);
...
body.cloud.addEventListener('click', () => {
  ...
  setTimeout(() => {
    body.clicker.classList.remove('clicked');
  }, 500);
});
